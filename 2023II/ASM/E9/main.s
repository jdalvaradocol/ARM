...
UNIDAD	 		EQU 	0x20000000
DECENA 			EQU 	0x20000004
...
;------------------------------------------
				...
				LDR		R11, = GPIOF_IDR	; Entrada GPIOF
				...
				MOV		R0, #0x00
				LDR		R1, = UNIDAD	; Cargo la posicion de memoria del registro
				STR		R0, [R1]
				
				MOV		R0, #0x00
				LDR		R1, = DECENA	; Cargo la posicion de memoria del registro
				STR		R0, [R1]

;------------------------------------------
				
				
LOOP			BL		DIS_DINAMICO

				LDR		R1, = UNIDAD	; Cargo la posicion de memoria del registro
				LDR		R0, [R1]		; LEER EL VALOR DE UNIDAD
...
DIS_DINAMICO	MOV		R3, LR
				MOV		R12, #5

LOOP_DIS		LDR		R1, = UNIDAD	; Cargo la posicion de memoria del registro
				LDR		R0, [R1]		; LEER EL VALOR DE UNIDAD
				BL		DISPLAY
				MOV		R0, R1
				BL		DISPLAYW
				...
				LDR		R1, = DECENA	; Cargo la posicion de memoria del registro
				LDR		R0, [R1]		; LEER EL VALOR DE DECENA				
				BL		DISPLAY
				MOV		R0, R1
				BL		DISPLAYW
